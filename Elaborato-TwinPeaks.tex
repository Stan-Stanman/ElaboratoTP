...
\begin{document}
\title{\includegraphics[width=12cm]{logoscuola.png}\\[2cm]\textbf{Da Cartesio a Philip Glass}}
...
\medskip
$= \frac{\sqrt{1 - \frac{v^2}{c^2}} \cdot (-c^2 - \frac{v^2}{1- \frac{v^2}{c^2}})}{c^4 \cdot (1- \frac{v^2}{c^2})} = \frac{(-c^2 - \frac{v \cdot c^2}{c^2 -v^2})}{c^4 \cdot \sqrt{(1- \frac{v^2}{c^2})}} = \frac{-c^4 + \cancel{c^2 \cdot v^2} - \cancel{c^2 \cdot v^2}}{c^2 - v^2} \cdot \frac{1}{c^3 \sqrt{c^2 - v^2}} = -\frac{c}{\sqrt{(c^2 - v^2)^3}}$ 
\end{center} 
...
Un esempio pratico ma efficace, per comprendere l’inadeguatezza in certi ambiti del modello classico è quello di un corpo in movimento alla velocità della luce che riflette la sua immagine su di uno specchio. Secondo il modello classico, non esistono velocità assolute ma i corpi si muovono con una velocità relativa rispetto ad un sistema di riferimento, ragione per il quale fu necessaria l’introduzione di un riferimento assoluto e soprattutto immobile rispetto al quale calcolare la velocità di ogni corpo appartenente all’ universo. Il quale, fu trovato  nell’\textit{etere}, il quinto elemento, \textit{la quinta essentia}, dopo \textit{il fuoco, l’acqua, la terra e l’aria}, che permea l’intero universo ed è immobile\cite{aristotele}, per l’appunto.  Quindi nel nostro caso, sia il corpo, sia la luce che emette e sia lo specchio, in quanto vincolato al corpo stesso, si muovono ad una velocità c rispetto all’etere. Applicando le trasformazioni galileane rispetto al corpo la velocita della sua luce è $v’= c – u$, ma essendo $u = c$, risulta che la velocità è nulla e che quindi non potrà mai riflettersi sullo specchio.
...
Supponendo di avere due sistemi $S$ ed $S’$ formati da tre assi e sia $v$ la velocità con cui $S'$ si allontana da $S$, sull'asse $x$. Ipotizzando un istante in cui i centri di origine $O$ e $O'$ coincidono e ponendo in questo istante come origine delle coordinate temporali $t=t'=0$, secondo Lorentz:\\
\medskip 
...
Fino ad allora, l’unico modo per stabilire se due eventi stessero avvenendo simultaneamente o meno era guardarli. Tant’è, che per un soggetto senziente basta essere nello stesso luogo in cui i due eventi avvengono, per stabilirne un rapporto: che sia esso di simultaneità o di causalità\cite{hume}.
...
E si immaginino due sistemi di riferimento $O$ ed $O’$ e due orologi, relativi rispettivamente ad un osservatore esterno in grado di guardare l’interno del vagone, e all’osservatore interno che fa partire i laser.
...
Se sottoponessimo allo stesso esperimento un emettitore di elettroni, potremmo stabilire con certezza la loro natura. Tuttavia, incredibilmente, i risultati ottenuti non sono quelli che ci aspettavamo. Infatti quando svolgiamo l'esperimento a luci spente, e quindi senza la possibilità di vedere gli elettroni stessi:
...
\setlength{\epigraphwidth}{0.8\textwidth}
\epigraph{\textit{Se si accetta che l'interpretazione della meccanica quantistica qui proposta sia corretta già in alcuni punti essenziali, allora dovrebbe essere permesso di affrontare in poche parole le conseguenze di principio. [...] nella formulazione netta del principio di causalità: "se conosciamo in modo preciso il presente, possiamo prevedere il futuro", non è falsa la conclusione, bensì la premessa. In linea di principio noi non possiamo conoscere il presente in tutti i suoi dettagli. [...] siccome tutti gli esperimenti sono soggetti alle leggi della meccanica quantistica e quindi all'equazione ${\displaystyle \Delta x\cdot \Delta p_{x}\,\sim \,h}$, mediante la meccanica quantistica viene stabilita definitivamente la non validità del principio di causalità.}}{Werner Karl Heisenberg, 1927}

Il principio di indeterminazione, segna la fine del determinismo meccanicistico, secondo il quale, l'uomo attraverso delle leggi possa predire, partendo da uno stato presente, uno stato futuro ad esso compatibile, come invece avevano sostenuto scienziati come Pierre Simon Laplace, Ruggero Giuseppe Boscovich e tanti altri fino ad allora.
L'avvento della meccanica quantistica, mutò completamente la situazione e con l'interpretazione ortodossa (non universalmente accettata) che fece Born, nel 1926, dell'equazione di Schrödinger, interpretando ${\displaystyle |\psi |^{2}}$, ovvero il modulo della funzione d'onda al quadrato, come legata alla distribuzione di probabilità della posizione dell'elettrone nello spazio, si aprì una vera e propria discussione filosofica e i sostenitori della teoria introdotta da Born formarono insieme la cosidetta \textit{scuola di Copenaghen}.
L'interpretazione di Copenaghen sostiene che, se sono vere le ipotesi di Born sulla funzione d'onda, un sistema fisico microscopico non possiede proprietà oggettive, prima che queste siano misurate e che quindi la meccanica quantistica fornisca solo un insieme di probabilità attribuibili al possibile esito della misura (probabilismo ontologico). 
...
È qui che inizia il suo \textit{Albedo} o sbiancamento, che corrisponde al progressivo risanamento della sua anima, testimoniato dal rito condotto da Virgilio su Dante a fine del I Canto. Questa fase è legata allegoricamente al luogo dove le anime soffrono ancora per le loro pene, ma hanno davanti a se la speranza della beatificazione.
...
Eppure l’uomo misura il tempo, stabilisce se un tempo è doppio, triplo, rispetto a un altro riferendosi inevitabilmente ad estensioni di tempo.
...
\begin{wrapfigure}{r}{0.4\textwidth}
  \centering
    \includegraphics[width=0.35\textwidth]{e1c291c26f0dabef705c23443c7b5c30.jpg}
  \caption{Philip Glass, dipinto di Chuck Close}
\end{wrapfigure}

\paragraph{La bellezza della ripetitività}
Se è vero che la misura del tempo, in quanto estensione dell’anima, non sia che la misura impressa nella nostra anima da un particolare evento e non della sua traccia passata, la ripetitività assume in questi termini tutt’altro valore, perché permetterebbe di ricreare all’interno della nostra anima un trama fitta di estensioni temporali, che si ripetono e si susseguono. Ed è proprio questo, l’elemento regnante nella musica di uno dei più grandi compositori e pianisti del XX secolo, Philip Glass, che attraverso il suo stile, ha rivoluzionato profondamente il mondo della musica, portando ad una delle massime espressioni, quello che oggi conosciamo come minimalismo musicale che ha ispirato compositori come Max Richter e Steve Reich.  Tuttavia, sarebbe semplicistico ridurre la musica di Glass a una mera ripetizione di suoni, perché in realtà la visione che ha il compositore dei pattern è molto più ampia e profonda, in quanto essi non costituiscono soltanto un completamento dell’armonia, ma diventano loro stessi l’armonia. La magia di Glass sta nel riuscire a tradurre in armonia anche una semplice ripetizione di numeri vocale, come in \textit{Einstein on the Beach, Knee 1-5} o una piccola successione di note. Una musica in cui tutto si muove e fa rumore ma in cui è la variazione il vero spartito.

\begin{wraptable}{l}{0.53\linewidth}
 \begin{tabular}{|l|}
  \hline
  \textbf{PCTO:} Il percorso PCTO svolto\\
   dalla classe si fonda principalmente  \\
   sullo scambio culturale con il liceo  \\
   Pujati di Sacile, attraverso sia la \\
   piattaforma eTwinning e sia uno scambio \\
   ravvicinato nelle nostre scuole. Il \\
   viaggio ha compreso, oltre alla scuola,\\
   anche la visita dei territori circostanti  \\
   come Venezia e Pordenone, ma anche di  \\
   alcune industrie storiche del posto, \\
   come la ben nota produttrice di  \\
   pianoforti Fazioli.\\
  \hline
 \end{tabular}
\end{wraptable}

\paragraph{Le metamorfosi}
Infatti accanto alla ripetitività, diventa di fondamentale importanza il tema della trasformazione, la metamorfosi, come evidente in \textit{Solo piano, Metamorphosis: 1-5}, che permette la creazione di una melodia. È la variazione continua a garantire continuità alle opere di Glass. Ogni pattern, ogni ripetizione si comporta come se fosse animata, nasce, si trasforma, muore, rinasce, si ritrasforma fino a morire di nuovo, un vortice continuo di bellezza che risulta difficile da spiegare, in quanto sconvolgente rispetto ai dogmi fino ad allora imposti. Inoltre, lo stile minimale, dato proprio dalla variazione e dall’assenza di chiari-scuri, rappresenta già di per se un modo di essere, e di vivere, che va oltre la musica stessa, spostando il contenuto dal testo che viene cantato, molte volte assente, nelle sensazioni e negli scenari che vengono evocati.
\begin{figure}[h]
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=1\textwidth]{cri_000000376455.jpg}
\caption{Cover Glassworks}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=1\textwidth]{2012eob0123001_new-1_600x600_1000KB.jpg}
\caption{Einstein on the Beach all'opera}
\end{subfigure}
\caption{Opere di Philip Glass}
\end{figure}
